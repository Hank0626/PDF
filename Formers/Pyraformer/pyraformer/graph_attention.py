...
class ProbSparseAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top): # n_top: c*ln(L_q)
        ...
        index_sample = torch.randint(L_K, (L_Q, sample_k)) # real U = U_part(predict_merge*ln(L_k))*L_q
        ...
        Q_reduce = Q[torch.arange(B)[:, None, None],
                     ...
                     M_top, :] # predict_merge*ln(L_q)
        Q_K = torch.matmul(Q_reduce, K.transpose(-2, -1)) # predict_merge*ln(L_q)*L_k
...
def parsing():
    ...
    parser.add_argument('-predict_merge', type=int, default=5)
